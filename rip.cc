...
Rip::Lookup(Ipv4Address dst, bool setSource, Ptr<NetDevice> interface)
{
    ...
    for (auto it = m_routes.begin(); it != m_routes.end(); it++)
    {
        ...
        if (j->GetRouteStatus() == RipRoutingTableEntry::RIP_VALID)
        {
            ...
            // Normalize both the lookup destination and stored route using the subnet mask
            // to ensure canonical (network + mask) comparison. This avoids mismatches when
            // routes contain non-zero host bits and aligns RIP route lookup with the routing refactoring (MR !2645).

            if (mask.IsMatch(dst.CombineMask(mask),entry.CombineMask(mask)))
            {
                ...
                if (!interface || interface == m_ipv4->GetNetDevice(j->GetInterface()))
                {
                    ...
                    rtentry->SetDestination(
                        route->GetDest().CombineMask(route->GetMask())
                    );
